...
/***/ function(module, exports, __webpack_require__) {
	...
	var _hazeFlight = __webpack_require__(13);
	
	var _hazeFlight2 = _interopRequireDefault(_hazeFlight);
	
	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }
	
	var canvas = document.querySelector('.Background-canvas');
	
	var textureAlign = { x: 0.1, y: 0.5 };
	var textures = [{
	  file: '../images/heat/flight.jpg',
	  name: 'image',
	  align: textureAlign,
	  scale: { x: 1, y: 1 }
	}, {
	  file: '../images/heat/flight-mask.jpg',
	  name: 'mask',
	  align: textureAlign,
	  scale: { x: 0.05, y: 0.05 }
	}, {
	  file: '../images/heat/flight-depth.jpg',
	  name: 'depth',
	  align: textureAlign,
	  scale: { x: 0.5, y: 0.5 }
	}, {
	  file: '../images/heat/noise.png',
	  name: 'noise',
	  repeat: true,
	  fill: false
	}];
	...
	var haze = new _haze2.default({
	  canvas: canvas,
	  shader: _hazeFlight2.default,
	  textures: textures
	});
	
	var contentTextureIndex = getTextureIndex();
	
	function loadContent(src, canvas) {
	  var scroll = arguments.length <= 2 || arguments[2] === undefined ? 0 : arguments[2];
	
	
	  var breakpoint = canvas.getAttribute('data-breakpoint');
	...
	var contents = [loadContent('../images/heat/air-travel-title.svg', document.querySelector('.Text-title')), loadContent('../images/heat/air-travel-title-medium.svg', document.querySelector('.Text-titleMedium'), 0.5), loadContent('../images/heat/air-travel-title-small.svg', document.querySelector('.Text-titleSmall'), 0.8)];
	...
	var getContent = function () {
	  ...
	  return function () {
	    ...
	    var content = ww != lastww ? contents.reduce(function (last, content) {
	      return window.innerWidth <= content.breakpoint ? content : last;
	    }) : lastContent;
	...
	haze.addEventListener('predraw', function (event) {
	  t += event.deltaTime;
	  haze.gl.createUniform('2f', 'parallax', Math.sin(t * 0.03) + Math.sin(t * 0.026), Math.sin(t * 0.04) + Math.sin(t * 0.012));
	});
	...
	function updateContent() {
	  var content = getContent();
	  if (content.loaded) {
	    var title = content.canvas;
	    var bb = title.getBoundingClientRect();
	    haze.gl.createUniform('2f', 'contentPos', bb.left * haze.dpi, (bb.top + (content.scroll != 0 ? document.body.scrollTop * content.scroll : 0)) * haze.dpi);
	  }
	}
